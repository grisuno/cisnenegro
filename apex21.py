...
# -*- coding: utf-8 -*-
"""
NeuroSovereign v15.4: Active Phase Intervention (Breaking the Ceiling)
Objective: Move from Observation to Causal Control.
Changes from v15.3:
1. ADDED: TopologyController to manage Phase Interventions.
2. LOGIC: If Phase is STABLE + Low Coarse Acc -> Inject Topological Noise (Reversibility).
3. ADDED: Stagnation Counter to trigger "Active Shocks".
4. FIX: Implemented causal intervention to break local minima in hierarchy learning.
"""
import torch
...
# Active Intervention (Reversibility)
STAGNATION_LIMIT = 10     # Epochs in STABLE before forcing intervention
TARGET_COARSE_V = 28.0    # Target to trigger intervention if not met
MIXER_NOISE_SCALE = 0.02  # Magnitude of topological perturbation
...
class PatchFeatureExtractor(nn.Module):
    ...
    def unfreeze_mixer_only(self):
        if self.use_mixer:
            ...
            print("      🔓 MIXER UNFROZEN (v15.4 Active Intervention)")
...
# =============================================================================
# 3. ACTIVE TOPOLOGY CONTROLLER (v15.4)
# =============================================================================
...
class TopologyController:
    """v15.4: Manages Active Interventions to break stagnation."""
    def __init__(self):
        self.stagnation_counter = 0

    def check_intervention(self, phase_state: str, coarse_acc: float, extractor) -> str:
        """
        ...
        if phase_state == "STABLE":
            if coarse_acc < TARGET_COARSE_V:
                ...
                self.stagnation_counter = 0 # Reset if target met
        else:
            self.stagnation_counter = 0 # Reset if shifting (progressing)

        if self.stagnation_counter > STAGNATION_LIMIT:
            action = "INTERVENE"
            self.perturb_mixer(extractor)
            self.stagnation_counter = 0 # Reset after intervention
            
        return action

    def perturb_mixer(self, extractor):
        """Causal Intervention: Inject topological noise to force phase shift."""
        if hasattr(extractor, 'mixer'):
            with torch.no_grad():
                for module in extractor.mixer.mixer:
                    if isinstance(module, nn.Linear):
                        # Inject Gaussian noise to weights
                        noise = torch.randn_like(module.weight) * MIXER_NOISE_SCALE
                        module.weight.add_(noise)
            return True
        return False

# =============================================================================
# 4. TAXONOMIC TRAINER (v15.4)
# =============================================================================
class TaxonomicTrainer:
    ...
    def train_single_chain(self, model, cycle, chain_type='APEX'):
        ...
        # v15.4: Reset controller for each run
        self.controller.stagnation_counter = 0
        ...
        for epoch in range(100):
            ...
            # v15.4: Active Intervention Check (Only APEX)
            intervention = "NONE"
            if chain_type == 'APEX':
                intervention = self.controller.check_intervention(phase_state, coarse_acc, extractor)
            ...
            if epoch % 20 == 0:
                ...
                if intervention == "INTERVENE":
                    log_str += f" ⚡ACTIVE_SHOCK" # Indicate causal intervention
...
def run_hierarchy_benchmark(model_apex, model_blind, device):
    ...
    if delta > 1.0:
        print("✅ VALIDATED: Active interventions improved structural plasticity.")
    else:
...
def main():
    print("=" * 90)
    print(" " * 25 + "NEUROSOVEREIGN v15.4: ACTIVE PHASE INTERVENTION")
    print("=" * 90)
    print("SOTA Features:")
    print("1. TopologyController manages Active Shocks (Reversibility).")
    print("2. Causal Noise Injection breaks STABLE stagnation.")
    print("3. Goal: Break the 27% Coarse Validation ceiling via Phase Control.")
    print("=" * 90)
    ...
    if os.path.exists('alpha_elk_v15_4_apex.pth'):
        print("📂 Loading Dual Alphas v15.4...")
        elk_state_apex = torch.load('alpha_elk_v15_4_apex.pth', map_location=device)
        elk_state_blind = torch.load('alpha_elk_v15_4_blind.pth', map_location=device)
        start_cycle = 1
    else:
        # Migración desde v15.3
        if os.path.exists('alpha_elk_v15_3_apex.pth'):
            print("📂 Migrating from v15.3 to v15.4 (Active Intervention)...")
            raw_apex = torch.load('alpha_elk_v15_3_apex.pth', map_location=device)
            raw_blind = torch.load('alpha_elk_v15_3_blind.pth', map_location=device)
            ...
            torch.save(elk_state_apex, 'alpha_elk_v15_4_apex.pth')
            torch.save(elk_state_blind, 'alpha_elk_v15_4_blind.pth')
            start_cycle = 1
        else:
            print("🥚 Generating Seed Elks v15.4 (Cycle 0)...")
            print("   🔎 Focus: Testing Active Intervention capability.")
            
            seed_apex = TaxonomicMLP(input_dim=INPUT_DIM, hidden_dim=FIXED_HIDDEN_DIM, num_classes=NUM_CLASSES).to(device)
            seed_blind = TaxonomicMLP(input_dim=INPUT_DIM, hidden_dim=FIXED_HIDDEN_DIM, num_classes=NUM_CLASSES).to(device)
            
            print("   - Seeding Apex (Intervention Active)...")
    ...
    if RUN_HIERARCHY_BENCHMARK:
        ...
        final_apex.load_state_dict(torch.load('alpha_elk_v15_4_apex.pth'))
        
        final_blind = TaxonomicMLP(input_dim=INPUT_DIM, hidden_dim=FIXED_HIDDEN_DIM, num_classes=NUM_CLASSES).to(device)
        final_blind.load_state_dict(torch.load('alpha_elk_v15_4_blind.pth'))
    ...
    print("\n" + "=" * 90)
    print(" " * 30 + "ACTIVE INTERVENTION REPORT v15.4")
    print("=" * 90)
    ...
    # v15.4: Validation Criteria (Ceiling Break)
    ceiling_broken = df['coarse_apex'].max() > 28.0
    
    if df['delta_hierarchy'].mean() > 1.0 and hierarchy_delta > 1.0:
        print("\n✅ CAUSAL CONTROL VALIDATED:")
        print("   Active Phase Interventions allowed sustained structural learning.")
        if ceiling_broken:
            print("   🚀 CEILING BROKEN: Coarse Acc exceeded 28.0% barrier.")
        else:
            print("   (Ceiling not yet fully broken, but trajectory improved)")
    else:
        print("\n⚠️  INTERVENTIONS INEFFECTIVE:")
        print("   Perturbations did not translate to better hierarchy encoding.")
