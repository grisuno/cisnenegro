...
# -*- coding: utf-8 -*-
"""
NeuroSovereign v15.5: Targeted Phase Surgery (Spectral Noise)
Objective: Fix "Blind Noise" critique by using Orthogonal Projection.
Changes from v15.4:
1. FIX: perturb_mixer() now uses Targeted Spectral Noise (Orthogonal to dominant subspace).
2. LOGIC: Intervention preserves dominant features while exciting latent modes.
3. MATH: Projection matrix P = I - V_dominant @ V_dominant.T ensures noise injection only in weak dimensions.
"""
import torch
...
# v15.5: TARGETED INTERVENTION PARAMS
LAMBDA_TAX_BASE = 0.1
...
# Active Intervention (Targeted)
STAGNATION_LIMIT = 10
TARGET_COARSE_V = 28.0
MIXER_NOISE_SCALE = 0.05     # Increased slightly as orthogonal noise is more subtle
DOMINANT_ENERGY_THRESHOLD = 0.8 # Keep top 80% of spectral energy intact
...
class PatchFeatureExtractor(nn.Module):
    ...
    def unfreeze_mixer_only(self):
        if self.use_mixer:
            ...
            print("      🔓 MIXER UNFROZEN (v15.5 Targeted Phase Surgery)")
...
# =============================================================================
# 3. TARGETED TOPOLOGY CONTROLLER (v15.5)
# =============================================================================
...
class TopologyController:
    """v15.5: Manages Targeted Spectral Interventions (Surgery)."""
    def __init__(self):
                ...
                for module in extractor.mixer.mixer:
                    if isinstance(module, nn.Linear):
                        ...
                        try:
                            ...
                            # Fallback to isotropic if SVD fails
                            module.weight.add_(torch.randn_like(W) * MIXER_NOISE_SCALE)
                            continue
                        
                        # 2. Identify Dominant Subspace (Top k components)
                        S_sq = S ** 2
                        ...
                        # 3. Create Projector onto Nullspace of Vh[:k]
                        # P = I - V_dominant @ V_dominant.T
                        V_dominant = Vh[:k, :].T # Shape [D_in, k]
                        # Projector: [D_in, D_in] = I - [D_in, k] @ [k, D_in]
                        # Optimized: We just need to project the noise vector
                        # Noise_orth = Noise - Noise @ V_dominant @ V_dominant.T
                        
                        random_noise = torch.randn_like(W)
                        
                        # Project noise to be orthogonal to dominant features
                        # N_proj = N - (N . V_d) . V_d^T
                        noise_proj = random_noise - (random_noise @ V_dominant) @ V_dominant.T
                        
                        # 4. Apply Targeted Noise
                        module.weight.add_(noise_proj * MIXER_NOISE_SCALE)
            return True
        return False

# =============================================================================
# 4. TAXONOMIC TRAINER (v15.5)
# =============================================================================
        ...
        for epoch in range(100):
            ...
            if epoch % 20 == 0:
                ...
                if intervention == "INTERVENE":
                    log_str += f" 🔧TARGETED_SHOCK" # v15.5 Indication
...
def run_hierarchy_benchmark(model_apex, model_blind, device):
    ...
    if delta > 1.0:
        print("✅ VALIDATED: Targeted interventions provided superior hierarchy encoding.")
    else:
        print("⚠️  WEAK SIGNAL: Targeted noise did not translate to lift.")
...
def main():
    print("=" * 90)
    print(" " * 25 + "NEUROSOVEREIGN v15.5: TARGETED PHASE SURGERY")
    print("=" * 90)
    print("SOTA Features:")
    print("1. TopologyController uses Orthogonal Spectral Projection.")
    print("2. Intervention preserves dominant modes, excites latent nullspace.")
    print("3. Goal: Surgical precision to break hierarchy ceiling.")
    print("=" * 90)
    ...
    if os.path.exists('alpha_elk_v15_5_apex.pth'):
        ...
        if os.path.exists('alpha_elk_v15_4_apex.pth'):
            print("📂 Migrating from v15.4 to v15.5 (Targeted Surgery)...")
            ...
            print("   🔎 Focus: Testing Targeted Spectral Intervention.")
            
            seed_apex = TaxonomicMLP(input_dim=INPUT_DIM, hidden_dim=FIXED_HIDDEN_DIM, num_classes=NUM_CLASSES).to(device)
            seed_blind = TaxonomicMLP(input_dim=INPUT_DIM, hidden_dim=FIXED_HIDDEN_DIM, num_classes=NUM_CLASSES).to(device)
            
            print("   - Seeding Apex (Targeted Surgery Active)...")
    ...
    print("\n" + "=" * 90)
    print(" " * 30 + "TARGETED SURGERY REPORT v15.5")
    print("=" * 90)
    ...
    if df['delta_hierarchy'].mean() > 1.0 and hierarchy_delta > 1.0:
        ...
        print("   Targeted Spectral Interventions preserved structure while expanding capability.")
        if ceiling_broken:
        ...
        print("\n⚠️  SURGERY UNSTABLE:")
        print("   Orthogonal noise did not yield expected hierarchy lift.")
