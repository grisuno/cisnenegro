...
# -*- coding: utf-8 -*-
"""
NeuroSovereign v14.5: Adaptive Shock & Full Spectrum Control
Objective: Fix v14.4 based on technical review.
Changes from v14.4:
1. FIX: Adaptive Taxonomic Shock (Lambda reacts to REAL validation gap, not epochs).
2. ADDED: Spectral Loss applied to Mixer weights (Upstream structural control).
3. ADDED: Feedback Loop Logic (Prev epoch gap determines current epoch lambda).
4. TUNED: Gap Threshold set to 5.0 for Shock activation.
"""
import torch
...
# v14.5: ADAPTIVE SHOCK PARAMS
LAMBDA_TAX_BASE = 0.1
LAMBDA_TAX_SHOCK = 0.6     # Activado dinámicamente si Gap > 5.0
GAP_SHOCK_THRESHOLD = 5.0  # Umbral de sobreajuste para activar shock
LAMBDA_SPARSE_MIXER = 1e-4
LAMBDA_SPECTRAL = 0.05     # Peso de la pérdida espectral (aplicado a FC1 y Mixer)
...
class PatchFeatureExtractor(nn.Module):
    ...
    def unfreeze_mixer_only(self):
        if self.use_mixer:
            ...
            print("      🔓 MIXER UNFROZEN (Active Evolution v14.5)")
...
# =============================================================================
# 4. TAXONOMIC TRAINER (v14.5: Adaptive Loop & Full Spectrum)
# =============================================================================
class TaxonomicTrainer:
    ...
    def train_single_chain(self, model, cycle, chain_type='APEX'):
        ...
        # v14.5: Configurar BLIND
        if chain_type == 'BLIND':
            model.fc_super.requires_grad_(False)
        
        extractor = self.extractor_apex if chain_type == 'APEX' else self.extractor_blind
        
        # v14.5: Feedback Loop Variable
        prev_gap = 0.0 # Inicialmente no hay gap previo (Epoch -1)
        
        for epoch in range(100):
            model.train()
            train_correct, train_total = 0, 0
            
            # --- v14.5: ADAPTIVE SHOCK LOGIC (START OF EPOCH) ---
            # Decidimos el lambda basado en el gap de la época ANTERIOR.
            ...
            # Feedback update for next iteration
            prev_gap = gap
            ...
            if epoch % 20 == 0:
                ...
                log_str += f" | Shock:{shock_state} | λ:{current_lambda_tax:.2f}"
                
                if chain_type == 'APEX':
                    # Calcular penalizaciones para reporte
                    m_spec_val = 0.0
                    m_sparse_val = 0.0
                    if hasattr(extractor, 'mixer'):
                        for m in extractor.mixer.mixer:
                            if isinstance(m, nn.Linear):
                                m_spec_val += compute_spectral_loss(m.weight).item()
                                m_sparse_val += m.weight.abs().sum().item()
                    
                    log_str += f" | L_FC1:{L:.3f} | L_Mix:{m_spec_val:.3f} | Spar:{spar:.1f}%"
...
def main():
    print("=" * 90)
    print(" " * 20 + "NEUROSOVEREIGN v14.5: ADAPTIVE SHOCK & FULL SPECTRAL")
    print("=" * 90)
    print("Fixes Implemented:")
    print("1. Lambda adapts dynamically to Real Validation Gap (Shock > 5.0).")
    print("2. Spectral Loss applied to both FC1 (Downstream) and Mixer (Upstream).")
    print("=" * 90)
    ...
    if os.path.exists('alpha_elk_v14_5_apex.pth'):
        print("📂 Loading Dual Alphas v14.5...")
        elk_state_apex = torch.load('alpha_elk_v14_5_apex.pth', map_location=device)
        elk_state_blind = torch.load('alpha_elk_v14_5_blind.pth', map_location=device)
        start_cycle = 1
    else:
        # Migración desde v14.4 si existe, si no genera
        if os.path.exists('alpha_elk_v14_4_apex.pth'):
            print("📂 Migrating from v14.4 to v14.5 (Adaptive Logic)...")
            raw_apex = torch.load('alpha_elk_v14_4_apex.pth', map_location=device)
            raw_blind = torch.load('alpha_elk_v14_4_blind.pth', map_location=device)
            ...
            torch.save(elk_state_apex, 'alpha_elk_v14_5_apex.pth')
            torch.save(elk_state_blind, 'alpha_elk_v14_5_blind.pth')
            start_cycle = 1
        else:
            print("🥚 Generating Seed Elks v14.5 (Cycle 0)...")
            seed_apex = TaxonomicMLP(input_dim=INPUT_DIM, hidden_dim=FIXED_HIDDEN_DIM, num_classes=NUM_CLASSES).to(device)
            seed_blind = TaxonomicMLP(input_dim=INPUT_DIM, hidden_dim=FIXED_HIDDEN_DIM, num_classes=NUM_CLASSES).to(device)
            
            # Cycle 0 es solo calentamiento
            print("   - Seeding Apex...")
    ...
    print("\n" + "=" * 90)
    print(" " * 30 + "SOTA CONTROL REPORT v14.5")
    print("=" * 90)
    ...
    # v14.5 Validation Criteria
    gap_stable = df['gap_apex'].iloc[-1] < 10.0
    hierarchy_learned = df['coarse_apex'].iloc[-1] > 30.0 # Un poco más exigente para validar shock
    
    if gap_stable and hierarchy_learned:
        print("\n✅ v14.5 VALIDATED:")
        print("   Adaptive Shock prevented runaway overfitting.")
        print("   Full Spectrum Loss stabilized both Mixer and MLP.")
    else:
        print("\n⚠️  OPTIMIZATION REVIEW:")
        print(f"   Gap Stable: {gap_stable} | Hierarchy Learned: {hierarchy_learned}")
