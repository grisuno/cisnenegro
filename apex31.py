...
5. [HARDENED] Updated torch.svd to torch.linalg.svd (future-proofing).
6. [HARDENED] Increased initialization std in GatedTokenMixer (v15.3 exploration logic).
...
class AdaptiveTopologyController:
    ...
    def detect_intervention_need(self, phase_state: str, extractor) -> Tuple[bool, str]:
        ...
        # Hysteresis: Do not intervene if currently shifting structure significantly
            ...
            with torch.no_grad():
                for module in extractor.mixer.mixer:
                    if isinstance(module, nn.Linear):
                        W = module.weight
                        try:
                            # HARDENED: Use torch.linalg.svd instead of deprecated torch.svd
                            U, S, Vh = torch.linalg.svd(W, full_matrices=False)
                        except:
...
class IterativeRefinementEngine:
    ...
    def apply_rank_capping(self, model: nn.Module, layer_name: str = 'fc1', keep_ratio: float = 0.85):
        ...
        W = layer.weight.data
        # HARDENED: Use torch.linalg.svd (future-proof)
        U, S, Vh = torch.linalg.svd(W)
        # Reconstruct V from Vh: V = Vh.T
        V = Vh.T
