...
# -*- coding: utf-8 -*-
"""
NeuroSovereign v15.4: Aggressive Hierarchical Shock & Warmup Sparsity
Based on v15.3 Architecture (Production-Ready)

Key Improvements for 30% Target:
1. [TUNED] LAMBDA_TAX_SHOCK increased to 0.7 (Stronger hierarchy forcing).
2. [TUNED] GAP_SHOCK_THRESHOLD lowered to 4.0 (Triggers intervention earlier).
3. [NEW] Mask Warmup: Prevents aggressive pruning in early epochs (improves baseline).
4. [FIX] Robust State Migration: Loads from v15.2 or v15.3 automatically.

Objective: Push the 24% initial baseline (via inheritance) to 30%+ structural advantage.
"""
import torch
...
# CONFIGURATION v15.4
# =============================================================================
NUM_EVOLUTION_CYCLES = 15  # Increased cycles for extended refinement
INPUT_DIM = 384
...
# v15.4: HYPERPARAMETERS (Aggressive Tuning)
LAMBDA_TAX_BASE = 0.15      # Increased from 0.1 to enforce hierarchy earlier
LAMBDA_TAX_SHOCK = 0.7      # Increased from 0.6 for massive structural shock
GAP_SHOCK_THRESHOLD = 4.0   # Lowered from 5.0 to trigger shock sooner
LAMBDA_SPARSE_MIXER = 1e-4
LAMBDA_SPECTRAL = 0.05

# Topology Monitoring (v15.3 inheritance)
PHASE_WINDOW = 5           
PHASE_STD_DEV_LIMIT = 1.5 

# Sparsity Control
MASK_WARMUP_EPOCHS = 20     # Do not apply hard sparsity until epoch 20
...
class PatchFeatureExtractor(nn.Module):
    def __init__(self, img_size=32, patch_size=4, in_chans=3, embed_dim=384, use_mixer=True):
        ...
        # v15.3 Initialization (preserved)
    ...
    def unfreeze_mixer_only(self):
        if self.use_mixer:
            ...
            print("      🔓 MIXER UNFROZEN (v15.4 Aggressive)")
...
# =============================================================================
# 2. TAXONOMIC LOTTERY TICKET MLP (CUDA Optimized Masks)
# =============================================================================
class TaxonomicMLP(nn.Module):
    def __init__(self, input_dim: int = 384, hidden_dim: int = 256, num_classes: int = 100):
        ...
        # Masks are buffers, so they move to GPU with model automatically
    ...
    def apply_masks(self):
        """Zero out weights based on masks. Runs on device (CUDA)."""
        with torch.no_grad():
...
class TaxonomicTrainer:
    ...
    def get_curriculum_dataset(self, cycle: int):
        ...
        sizes = {1: 5000, 4: 20000, 7: 50000, 10: 50000}
    ...
    def train_single_chain(self, model, cycle, chain_type='APEX'):
        ...
        for epoch in range(100):
            ...
            # v15.4: Aggressive Shock Trigger
            ...
            for inputs, labels in trainloader:
                ...
                # v15.4: Gradient Masking (Standard)
                ...
                # v15.4: Mask Warmup Logic
                # Don't apply hard pruning until the network has learned basic features
            ...
            if epoch % 20 == 0:
                ...
                log_str = f"   [{chain_type}] Ep {epoch:3d} | V:{test_acc:5.1f}% | C.V:{coarse_acc:5.1f}% | Gap:{gap:4.1f}"
        ...
        if best_state: 
            ...
            model.apply_masks() # Final mask application
...
def run_hierarchy_benchmark(model_apex, model_blind, device):
    ...
    # Use the SAME extractors for fair comparison
    ...
    def evaluate(model, extractor, name):
        ...
        with torch.no_grad():
            for inputs, labels in testloader:
                ...
                # Manual forward pass ensuring masks are active
    ...
    print("-" * 90)
    if delta > 1.0: print("✅ VALIDATED: Hierarchy signal is structurally induced.")
    else: print("⚠  WEAK SIGNAL: Structural advantage is minimal.")
    return delta
...
def main():
    print("=" * 90)
    print(" " * 25 + "NEUROSOVEREIGN v15.4: AGGRESSIVE SHOCK")
    print("=" * 90)
    print("Features:")
    print("- Increased LAMBDA_TAX (0.15 Base / 0.7 Shock)")
    print("- Mask Warmup (20 epochs)")
    print("- Low Gap Threshold (4.0) for faster reaction.")
    print("=" * 90)
    ...
    # ============================
    # 1. DUAL ALPHA LOADING
    # ============================
    # Priority: v15.3 -> v15.2 -> Scratch
    if os.path.exists('alpha_elk_v15_3_apex.pth'):
        print("📂 Loading Dual Alphas v15.3 (Warm Start Active)...")
        ...
        start_cycle = 1
    elif os.path.exists('alpha_elk_v15_2_apex.pth'):
        print("📂 Migrating from v15.2 to v15.4...")
        ...
        print("🥚 Generating Seed Elks v15.4 (Cold Start)...")
        seed_apex = TaxonomicMLP(input_dim=INPUT_DIM, hidden_dim=FIXED_HIDDEN_DIM, num_classes=NUM_CLASSES).to(device)
        seed_blind = TaxonomicMLP(input_dim=INPUT_DIM, hidden_dim=FIXED_HIDDEN_DIM, num_classes=NUM_CLASSES).to(device)
    ...
    for cycle in range(start_cycle, NUM_EVOLUTION_CYCLES + 1):
        ...
        evolution_log.append({
            'cycle': cycle, 'acc_apex': res_apex['final_acc'], 'acc_blind': res_blind['final_acc'],
            'coarse_apex': res_apex['final_coarse_acc'], 'coarse_blind': res_blind['final_coarse_acc'],
            'delta_structure': delta_structure, 'delta_hierarchy': delta_hierarchy,
            'gap_apex': res_apex['final_gap'], 'gap_blind': res_blind['final_gap'],
            'topo_ratio_apex': res_apex['topo_ratio'], 'topo_ratio_blind': res_blind['topo_ratio']
        })
    ...
    print("\n" + "=" * 90)
    print(" " * 30 + "v15.4 REPORT: AGGRESSIVE SHOCK")
    print("=" * 90)
    ...
    print(f"   Benchmark:   {hierarchy_delta:+.2f}%")
    
    if df['delta_hierarchy'].mean() > 2.0:
        print("\n✅ STRONG VALIDATION: Aggressive Shock successfully boosted hierarchy.")
    elif df['delta_hierarchy'].mean() > 1.0:
        print("\n✅ VALIDATED: Hierarchy signal present.")
    else:
        print("\n⚠  WEAK SIGNAL.")
