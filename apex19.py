...
# -*- coding: utf-8 -*-
"""
NeuroSovereign v15.2: Topological Phase Detection
Objective: Detect Grokking moments via Topological Ratio (L_opt / L_mon).
Changes from v15.1:
1. ADDED: TopologicalMonitor to track Phase Shift (Ratio R = L_opt / L_mon).
2. LOGIC: Identification of "Stagnation Phase" vs "Plasticity Phase".
3. FOCUS: Run Cycle 0 (Seeding) to prove Architectural Necessity vs Regularization sufficiency.
"""
import torch
...
# Topology Monitoring
R_TOPOLOGY_CRITICAL = 0.1  # Threshold for "Phase Shift" warning
...
class PatchFeatureExtractor(nn.Module):
    ...
    def unfreeze_mixer_only(self):
        if self.use_mixer:
            ...
            print("      🔓 MIXER UNFROZEN (v15.2 Topological Phase)")
...
class SpectralMonitor:
    ...
    def compute_topology_ratio(self, model, extractor, chain_type):
        """
        v15.2: Calcula el ratio R = L_opt / L_mon.
        Valores bajos indican alineación estable.
        Valores altos o erráticos indican transición de fase (Grokking/Collapse).
        """
        # 1. Calcular L_opt (Función de costo activa)
        L_opt_val = compute_spectral_loss(model.fc1.weight)
        if hasattr(extractor, 'mixer'):
            for m in extractor.mixer.mixer:
                if isinstance(m, nn.Linear):
                    L_opt_val += compute_spectral_loss(m.weight)
        
        # 2. Calcular L_mon (Métrica de reporte)
        L_mon_val, _, _ = self.compute_metrics(model.fc1.weight)
        
        # Ratio
        ratio = (L_opt_val.item() / L_mon_val) if L_mon_val > 0 else 0.0
        return ratio, L_opt_val.item(), L_mon_val

# =============================================================================
# 4. TAXONOMIC TRAINER (v15.2)
# =============================================================================
class TaxonomicTrainer:
    ...
    def train_single_chain(self, model, cycle, chain_type='APEX'):
        ...
        # v15.2: Historial para detección de fases
        ratio_history = []
        ...
        for epoch in range(100):
            ...
            for inputs, labels in trainloader:
                ...
                if chain_type == 'APEX':
                    ...
                    # v15.2: BLIND con Symmetric Spectral Loss
            ...
            # v15.2: Cálculo de métricas topológicas
            topo_ratio, l_opt_val, l_mon_val = self.monitor.compute_topology_ratio(model, extractor, chain_type)
            ...
            if epoch % 20 == 0:
                ...
                # Detección de fase
                phase = "STABLE"
                if topo_ratio > R_TOPOLOGY_CRITICAL:
                    phase = "SHIFTING" # Symmetry breaking
                ...
                log_str += f" | Topo_R:{topo_ratio:.3f}({phase}) | L_opt:{l_opt_val:.3f} | Spar:{spar:.1f}%"
        ...
        avg_ratio = np.mean(ratio_history[-10:]) # Media de las últimas 10 épocas
        return {
            'final_acc': best_acc, 
...
def run_hierarchy_benchmark(model_apex, model_blind, device):
    ...
    def evaluate(model, extractor, name):
        ...
        with torch.no_grad():
            for inputs, labels in testloader:
                ...
                outputs_super = model.fc_super(features * model.mask1) 
    ...
    if delta > 1.0:
        ...
        print("⚠️  WEAK SIGNAL: Structural advantage is minimal.")
...
def main():
    print("=" * 90)
    print(" " * 25 + "NEUROSOVEREIGN v15.2: TOPOLOGICAL PHASE DETECTION")
    print("=" * 90)
    print("Updates:")
    print("1. Added Topological Ratio (Topo_R) to detect symmetry breaking.")
    print("2. BLIND Seeding confirms Architectural Necessity.")
    print("=" * 90)
    ...
    # ============================
    # 1. INITIALIZE DUAL ELKS (Seeding Cycle 0)
    # ============================
    if os.path.exists('alpha_elk_v15_2_apex.pth'):
        print("📂 Loading Dual Alphas v15.2...")
        elk_state_apex = torch.load('alpha_elk_v15_2_apex.pth', map_location=device)
        elk_state_blind = torch.load('alpha_elk_v15_2_blind.pth', map_location=device)
        start_cycle = 1
    else:
        # Migración desde v15.1
        if os.path.exists('alpha_elk_v15_1_apex.pth'):
            print("📂 Migrating from v15.1 to v15.2 (Topology)...")
            raw_apex = torch.load('alpha_elk_v15_1_apex.pth', map_location=device)
            raw_blind = torch.load('alpha_elk_v15_1_blind.pth', map_location=device)
            ...
            torch.save(elk_state_apex, 'alpha_elk_v15_2_apex.pth')
            torch.save(elk_state_blind, 'alpha_elk_v15_2_blind.pth')
            start_cycle = 1
        else:
            print("🥚 Generating Seed Elks v15.2 (Cycle 0)...")
            print("   🔎 Focus: Detecting Phase Shift during Seeding.")
            
            seed_apex = TaxonomicMLP(input_dim=INPUT_DIM, hidden_dim=FIXED_HIDDEN_DIM, num_classes=NUM_CLASSES).to(device)
            seed_blind = TaxonomicMLP(input_dim=INPUT_DIM, hidden_dim=FIXED_HIDDEN_DIM, num_classes=NUM_CLASSES).to(device)
            
            print("   - Seeding Apex (Plastic Phase expected)...")
            res_apex = trainer.train_single_chain(seed_apex, 0, 'APEX')
            print(f"   - Final Topo Ratio (APEX): {res_apex['topo_ratio']:.4f}")
            
            print("   - Seeding Blind (Stagnation Phase expected)...")
            res_blind = trainer.train_single_chain(seed_blind, 0, 'BLIND')
            print(f"   - Final Topo Ratio (BLIND): {res_blind['topo_ratio']:.4f}")
    ...
    if RUN_HIERARCHY_BENCHMARK:
        ...
        final_apex.load_state_dict(torch.load('alpha_elk_v15_2_apex.pth'))
        
        final_blind = TaxonomicMLP(input_dim=INPUT_DIM, hidden_dim=FIXED_HIDDEN_DIM, num_classes=NUM_CLASSES).to(device)
        final_blind.load_state_dict(torch.load('alpha_elk_v15_2_blind.pth'))
    ...
    print("\n" + "=" * 90)
    print(" " * 30 + "TOPOLOGICAL PHASE REPORT v15.2")
    print("=" * 90)
    ...
    print(f"   Avg Topo_R (APEX): {df['topo_ratio_apex'].mean():.4f}")
    print(f"   Avg Topo_R (BLIND): {df['topo_ratio_blind'].mean():.4f}")
    
    # Interpretación de fases
    if df['topo_ratio_blind'].mean() < 0.05 and df['coarse_blind'].mean() < 5.0:
        print("\n✅ ARCHITECTURAL NECESSITY CONFIRMED:")
        print("   BLIND is topologically stable but semantically empty.")
        print("   Regularization alone cannot compensate for lack of structural channels.")
    else:
        print("\n⚠️  UNEXPECTED BEHAVIOR:")
        print("   BLIND is showing activity beyond baseline noise.")
